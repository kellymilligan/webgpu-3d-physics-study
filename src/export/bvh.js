// A compact Morton-ordered BVH. Leaves retain original Box3D body IDs/materials.
export function buildBVH(state) {
  const count=state.length/16;
  if(!Number.isInteger(count)||count<1)throw new Error('Empty or invalid body snapshot');
  const positions=new Float32Array(state),codes=new Uint32Array(count);
  let ids=new Uint32Array(count),scratch=new Uint32Array(count);
  const lo=[Infinity,Infinity,Infinity],hi=[-Infinity,-Infinity,-Infinity];
  for(let i=0;i<count;i++)for(let k=0;k<4;k++){
    const v=state[i*16+k];if(!Number.isFinite(v)||(k===3&&v<=0))throw new Error('Invalid body in snapshot');
    if(k<3){lo[k]=Math.min(lo[k],v);hi[k]=Math.max(hi[k],v);}
  }
  ...
  for(let i=0;i<count;i++){
    const q=lo.map((v,k)=>Math.min(1023,Math.max(0,Math.floor((positions[i*16+k]-v)*scale[k]))));
  ...
  function node(first,last){
    ...
    if(last-first===1){
      ...
      for(let j=start;j<end;j++){const p=ids[j]*16,r=positions[p+3];for(let k=0;k<3;k++){f[o+k]=Math.min(f[o+k],positions[p+k]-r-1e-5);f[o+4+k]=Math.max(f[o+4+k],positions[p+k]+r+1e-5);}}
    }else{
