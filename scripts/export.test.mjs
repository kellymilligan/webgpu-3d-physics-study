...
test('BVH includes all IDs and conservatively bounds every sphere',()=>{
 const state=new Float32Array(10003*12);for(let i=0;i<state.length/12;i++)state.set([Math.sin(i)*10,(i%37)*.2,Math.cos(i*1.7)*5,.02+(i%19)*.01],i*12);
 ...
 function check(index,depth=0){assert.ok(depth<48);const o=index*8;
  if(u[o+7]&0x80000000){for(let j=0;j<(u[o+7]&0x7fffffff);j++){const id=result.indices[u[o+3]+j];found.push(id);for(let k=0;k<3;k++){assert.ok(f[o+k]<=state[id*12+k]-state[id*12+3]+2e-6);assert.ok(f[o+4+k]>=state[id*12+k]+state[id*12+3]-2e-6);}}}
...
test('BVH ray traversal agrees with a brute-force sphere reference',()=>{
 const n=513,state=new Float32Array(n*12);for(let i=0;i<n;i++)state.set([Math.sin(i*.7)*8,Math.cos(i)*5,Math.sin(i*1.31)*7,.1+(i%11)*.03],i*12);
 const bvh=buildBVH(state),f=new Float32Array(bvh.nodes),u=new Uint32Array(bvh.nodes);
 function sphere(id,o,d){const p=id*12,oc=o.map((v,k)=>v-state[p+k]),b=oc.reduce((sum,v,k)=>sum+v*d[k],0),h=state[p+3]**2-oc.reduce((sum,v,k)=>sum+(v-b*d[k])**2,0);if(h<0)return Infinity;const a=-b-Math.sqrt(h),c=-b+Math.sqrt(h);return a>0?a:c>0?c:Infinity;}
